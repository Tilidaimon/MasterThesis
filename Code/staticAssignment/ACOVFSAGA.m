% 基于粒子退火遗传的目标分配算法（ACOVFSAGA） TODO
% 暂时先用竞争奖励机制看看效果

% 模拟退火遗传算法，退火对象为变异算子和交叉算子
function [planSAGA, fBestSave,fMeanSave, time, XcrossSave,XmutateSave] = ACOVFSAGA(model,popsize,steps,Pcross,Pmutate,Top,Dop)
% 输入obj，模型
...
Xcross = [1 1 1 1 1]*0.2; % 交叉算子信息素
Xmutate = ones(1,7)*1/7;  % 变异算子信息素
...
% 计算适应度函数
for i=1:popsize
    fitPop(i) = GlobalUtility(model, s(i,:));
end
%% 主循环开始
for step=1:steps
   % 先保存一些重要的变量
    ...
    [sCross,fitPop,Xcross] = cross(model,sSelect, s, fitPop, Pcross,Xcross); % 交叉,同时更新适应度等
    XcrossSave(step,:) = Xcross;
    [sMutate,fitPop,Xmutate] = mutate(model, sCross, fitPop, Pmutate,Xmutate);% 变异
    XmutateSave(step,:) = Xmutate;
    r = randperm(popsize,1);
    sMutate(r,:) = bestPlan; % 最优解取回来
    fitPop(r) = oldFit(maxIndex);
    sMutate(1,:) = bestPlan; % 最优解取回来
    fitPop(1) = oldFit(maxIndex);
...
upNum = ones(1,5); % 新解更好的个数，默认都不好时候平均化
...
%         crossHandle = @crossPMX; % DEBUG
        ...
        % 计算交叉之后是否产生了更好的解
        oldF = fitPop(i); % 老的解
        fitPop(i) = GlobalUtility(obj, sCross(i,:));   % 更新新的函数值
        if fitPop(i)>=oldF
...
%% 群体变异
function [sMutate,newFit,newX] = mutate(obj, s, fitPop, Pmutate,X)
...
%          mutateHandle % DEBUG
%          mutateHandle = @mutateEM; %DEBUG
        sMutate(i,:) = mutateHandle(s(i,:)); % 算子选择     
        % 计算交叉之后是否产生了更好的解   
	    oldF = fitPop(i); % 老的解
        fitPop(i) = GlobalUtility(obj, s(i,:));
        if  fitPop(i)>=oldF
            upNum(p)=upNum(p)+1;
        end
    end

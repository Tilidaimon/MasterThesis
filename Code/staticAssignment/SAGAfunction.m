...
function [planSAGA,sNew,fBestSave,fMeanSave, time] = SAGAfunction(s,model,popsize,steps,Pcross,Pmutate, crossAlg, mutateAlg,Top,Dop)
% 这个算法多了一个初始种群的参数
% 输入model，模型
...
%       fBestSave ，每一步最好的解记录
%       fMeanSave , 每一步平均解记录
% test: [planSAGA, fBestSave,fMeanSave, time] = GA1(model,100,100,1,0.5,"PMX","EM")
if nargin < 7
...
for step=1:steps
   % 先保存一些重要的变量
    ...
    oldFit = fitPop;
    for i=1:popsize
%         plan = decodeFromHA(model,sMutate(i,:));  % 这两步是具体问题的计算吗，其它形式问题修改这个部分即可
        fitPop(i) = GlobalUtility(model, sMutate(i,:));   % ---------------------
        % 退火

...
JSFP_mean_max = zeros(Mcur,Ncur);
GRM_mean_max = zeros(Mcur,Ncur);
SAP_mean_max = zeros(Mcur,Ncur);
PSO_mean_max = zeros(Mcur,Ncur);
ACO_mean_max = zeros(Mcur,Ncur);
SAGA_mean_max = zeros(Mcur,Ncur);
GA_mean_max = zeros(Mcur,Ncur);
IGA_mean_max = zeros(Mcur,Ncur);
...
for Nm = 10:10:100
    for Nt = 10:10:100
        ...
        indexM = Nm/10;
        indxN = Nt/10;
        JSFP_mean_max(indexM,indexN) = mean(JSFP_max);
        GRM_mean_max(indexM,indexN) = mean(GRM_max);
        SAP_mean_max(indexM,indexN) = mean(SAP_max);
        PSO_mean_max(indexM,indexN) = mean(PSO_max);
        ACO_mean_max(indexM,indexN) = mean(ACO_max);
        SAGA_mean_max(indexM,indexN) = mean(SAGA_max);
        GA_mean_max(indexM,indexN) = mean(GA_max);
        IGA_mean_max(indexM,indexN) = mean(IGA_max);
    end
end

algo_cell = {'JSFP';'GRM';'SAP';'ACO';'PSO';'GA';'IGA';'SAGA'};
[l,~]=size(algo_cell);
filename = '/TestMaxwithNum/';
for i=1:l
